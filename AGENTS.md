...
2. **Notarized DMG requires Developer ID** — Until then, default to interim **ad-hoc** artifacts via `scripts/package-adhoc-release.sh` and attach DMG/ZIP to GitHub Releases with clear non-notarized notes. Ad-hoc packaging does not submit binaries to Apple notarization and does not add quota telemetry. Do not enable `ENABLE_NOTARIZED_RELEASE=1` yet. Company-paid certificates are out of scope; a personal Apple Developer Program membership may replace ad-hoc later.
