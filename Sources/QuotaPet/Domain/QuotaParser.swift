...
enum QuotaParser {
    ...
    private static func bucketMap(in root: [String: Any]) -> [String: [String: Any]] {
        let keys = ["rateLimitsByLimitId", "rate_limits_by_limit_id", "rateLimits", "rate_limits"]
        for key in keys {
            guard let rawBuckets = root[key] as? [String: Any] else { continue }
            let buckets = rawBuckets.compactMapValues { $0 as? [String: Any] }
            if !buckets.isEmpty {
                return buckets
            }
        }
    ...
    private static func window(in object: [String: Any], bucketID: String, role: String) -> QuotaWindow? {
        ...
        let id = "\(bucketID)|\(boundedRole)|\(windowDurationMinutes)|\(resetComponent)"
    ...
    private static func number(in object: [String: Any], keys: [String]) -> Double? {
        keys.compactMap { object[$0] as? NSNumber }.first.map { $0.doubleValue }
    }

    private static func integer(in object: [String: Any], keys: [String]) -> Int? {
        keys.lazy.compactMap { object[$0] as? NSNumber }.first.flatMap { number in
